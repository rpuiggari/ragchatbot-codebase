...
class RAGSystem:
    ...
    def query(
        ...
        # Use sequential tool calling if enabled and available
        if (
            hasattr(self.config, "ENABLE_SEQUENTIAL_TOOLS")
            and self.config.ENABLE_SEQUENTIAL_TOOLS
        ):
            max_rounds = getattr(self.config, "MAX_TOOL_ROUNDS", 2)
            ...
            # Fall back to single tool calling
        ...
        try:
            ...
            print(f"Warning: Could not retrieve sources from tool manager: {e}")
            sources = []
        ...
        try:
            ...
            print(f"Warning: Could not reset sources in tool manager: {e}")
            pass
